...
const QUICK_LINKS = [
  ...
  ["Google Translate", "https://translate.google.com", "translate.google.com", "G文"],
  ["DeepL Translate", "https://www.deepl.com/translator", "deepl.com", "文"],
...
try {
  const saved = JSON.parse(localStorage.getItem('nyitany-v1'));
...
if (!['hu', 'en'].includes(prefs.language)) {
  prefs.language = 'hu';
}
...
document
  .querySelectorAll('[placeholder],[aria-label]')
  .forEach(el =>
...
function translateUI() {
  ...
  translatedNodes.forEach(({ node, key }) => {
    node.textContent = t(key);
  });

  translatedAttrs.forEach(({ el, attr, key }) => {
...
function save() {
  try {
    ...
    toast(
      t("A böngésző nem engedi a beállítások mentését.")
    );
  }
}
...
function tick() {
  ...
  $('phase-label').textContent =
    {
      morning: t("☀ Reggeli fények"),
      day: t("☀ Nappali nyugalom"),
      evening: t("◒ Esti hangulat"),
      night: t("☾ Éjszakai csend")
    }[phase] +
    ' · ' +
    (
      prefs.theme === 'auto'
        ? t("automatikus háttér")
        : t("választott háttér")
    );
...
/*
=========================================================
GYORSLINKEK MEGJELENÍTÉSE
=========================================================

Az ikonok már nem karakterek.

A Google favicon szolgáltatásából töltjük be az adott
weboldal valódi faviconját.
...
Ha egy ikon valamiért nem töltődik be, akkor jelenik csak
meg a korábbi szöveges tartalék.
*/
...
function render() {
  ...
  $('engine-label').textContent =
    t('Kereső: ') +
    (engines[prefs.engine] || engines.google)[0];
  ...
  QUICK_LINKS.forEach(item => {
    ...
    /*
    Ha azt szeretnéd, hogy ugyanabban a böngészőfülben
    nyíljon meg, akkor ezt a két sort törölheted.
    */
    ...
    /*
    Ikon külső konténere.
    Meghagyjuk az eredeti "app-icon" CSS osztályt,
    így az oldal meglévő dizájnja továbbra is működik.
    */
    ...
    */
    ...
    icon.className = 'app-icon-image';

    /*
    Dekoratív kép, ezért az alt üres.
    A link nevét külön kiírjuk.
    */
    ...
    */
    ...
    /*
    Google favicon szolgáltatás.

    A domain alapján automatikusan az adott oldal
    faviconját kéri le.
    */
    ...
    /*
    Ha valamelyik favicon nem tölthető be,
    visszaállunk a régi karakteres ikonra.
    */
...
$('search').onsubmit = e => {
  ...
  if (q) {
    location.href =
      (engines[prefs.engine] || engines.google)[1] +
...
$('settings-open').onclick = () => {
  ...
  $('calendar-input').value =
    config.googleCalendarEmbedUrl || '';

  $('banner-one-input').value =
    config.myFlightRadar24Banners?.[0]?.imageUrl || '';

  $('banner-two-input').value =
    config.myFlightRadar24Banners?.[1]?.imageUrl || '';
...
$('preferences').onsubmit = e => {
  ...
  const calendar =
    $('calendar-input').value.trim();

  const images = [
    $('banner-one-input').value.trim(),
    $('banner-two-input').value.trim()
  ];
  ...
  if (
    ...
    $('settings-error').textContent =
      t(
        'A Google Naptár beágyazási címe nem megfelelő.'
      );
  ...
  if (
    ...
    $('settings-error').textContent =
      t(
        'A bannerhez érvényes HTTPS-képcím szükséges.'
      );
...
$('city-form').onsubmit =
  async e => {
    ...
    try {
      ...
      $('city-status').textContent =
        data.results?.length
          ? t("Válaszd ki a települést:")
          : t(
              "Nincs találat. Próbálj más településnevet."
            );


      for (
        const city of data.results || []
      ) {
      ...
      if (
        ...
        $('city-status').textContent =
          t(
            "A keresés most nem érhető el. Ellenőrizd az internetkapcsolatot, majd próbáld újra."
          );
      }
    }
  };
...
async function weather() {
  ...
  try {
    ...
    const data =
      await fetchJSON(
        `https://api.open-meteo.com/v1/forecast?latitude=${city.latitude}&longitude=${city.longitude}&current=temperature_2m,weather_code&daily=temperature_2m_max,temperature_2m_min&timezone=auto&forecast_days=1`
      );
    ...
    $('condition').textContent =
      t(
        "Az időjárás most nem érhető el."
      );
...
/*
Ha visszajön az internet,
frissítjük az időjárást.
*/
  ...
  if (url) {
    ...
    emptyEmbed(
      ...
      config.googleCalendarEmbedUrl
        ...
        : t(
            'Add meg a naptár linkjét a Testreszabás menüben.'
          ),
  ...
  [0, 1].forEach(i => {
    ...
    if (!src) {
      ...
      emptyEmbed(
        ...
        t(
          'Add meg a kép linkjét a Testreszabás menüben.'
        ),
    ...
    img.onerror = () => {
      ...
      if (
        ...
        container.textContent =
          t(
            'A banner nem tölthető be. Ellenőrizd a képcímet és az internetkapcsolatot.'
          );
      }
    };
